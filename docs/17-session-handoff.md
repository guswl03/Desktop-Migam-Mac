...
6. 설정에서 CPU·메모리·통합을 각각 저장하고 실시간 수치와 펫 속도/표정 변화를 확인한다.
7. CPU 90% 이상 전용 과열·땀 자산이 필요하면 4~6프레임으로 추가한다.
8. 펫 우클릭 팝업이 작업표시줄·화면 가장자리에서 잘리지 않는지, 버튼별 창 전환이 자연스러운지 확인한다.
9. 알림 영역에 CPU 파랑·MEM 빨강 아이콘이 각각 표시되고 부하 변화에 따라 게이지·표정·툴팁이 갱신되는지 확인한다. `^` 안에 있으면 작업표시줄로 끌어낸다.
10. 설정의 시스템 반응 기준을 CPU/MEM/통합으로 바꾸고 20·40·60·80% 경계에서 alert/medium/fast/extreme 이미지, 이동 속도, 왼쪽 반전과 바닥 접지를 확인한다.
